...
5. Optionally add `coverFit: cover` to make the cover fill its tile box and crop, instead of the default `contain`, which shows the whole frame. Worth setting when the cover's aspect does not match the tile's — a 16:9 clip letterboxes inside the square box big desktops use, and fills under `cover`. Any value other than `cover`/`contain` throws at build. `coverPosition` (`center`, `top`, `bottom`, `left`, `right`) picks which part survives the crop, for covers whose subject is not centred.
